...
if os.getenv("private_key") and os.getenv("client_email"):
    ...
    # 로컬 개발 환경: GOOGLE_APPLICATION_CREDENTIALS 환경변수 사용
    db = firestore.Client()
...
def update_excel_for_cert(user_uid, cert_id, cert_info):
    ...
    if user_snapshot.exists:
        ...
        user_name  = user_data.get("name", "")      # 예: Firestore에 저장된 'name' 필드
        user_phone = user_data.get("phone", "")     # 예: Firestore에 저장된 'phone' 필드
        user_email = user_data.get("email", "")     # 예: Firestore에 저장된 'email' 필드
    else:
        user_name  = ""
        user_phone = ""
        user_email = ""
    ...
    print(f"[EXCEL UPDATE] cert {cert_id} for user {user_uid} added at {updated_date}.")

    # --- 7) Firestore 문서에 excelUpdated=True로 표시 ---
